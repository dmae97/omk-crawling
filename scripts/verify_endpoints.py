...
def main() -> None:
    ...
    def bm_gateway():
        ...
        bucket.acquire()
        r = cffi.get(
            "https://gateway-api.baemin.com/v1/tabs/review",
    ...
    def bm_search():
        ...
        bucket.acquire()
        r = cffi.get(
            "https://search-gateway.baemin.com/v1/search",
    ...
    def bm_webview():
        ...
        bucket.acquire()
        r = cffi.get(
            "https://web.baemin.com/food/shops",
    ...
    def test_rotator():
        ...
        next_imp = rot.next()
        return {
            "ok": len(seen) >= 4 and next_imp != "chrome124",
    ...
    def test_headers():
        ...
        return {
            "ok": "User-Agent" in h and h.get("X-Custom") == "test",
    ...
    def test_pw():
        ...
        return {
            "ok": True,
            "detail": (
                "installed" if ready else "not installed (run: playwright install chromium)"
            ),
        }
    ...
    def test_cafe_list():
        ...
        return {
            ...
            "status": r.status_code,
            "detail": (
                f"{len(items)} articles, first='{items[0]['subject'][:20]}'" if items else "empty"
            ),
        }
    ...
    def test_breaker():
        ...
        for _ in range(2):
            # Deliberately tripping the breaker — the raise IS the test.
    ...
    def test_encoding():
        ...
        cc = CapturedCall(
            url="t", method="GET", status=200, response_bytes="배달의민족".encode("cp949")
        )
        out = cc.decoded_text()
    ...
    def test_adaptive():
        ...
        f.close()
        return {
            "ok": r.ok and r.json_data is not None,
